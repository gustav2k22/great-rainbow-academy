...
interface Campaign { id: string; subject: string; preview: string | null; body: string | null; status: string; sent_at: string | null; recipients: number | null; created_at: string }
interface Subscriber { id: string; email: string; full_name: string | null; status: string; created_at: string }

export function NewsletterManager({ campaigns, subscribers }: { campaigns: Campaign[]; subscribers: Subscriber[] }) {
  ...
  function send(c: Campaign) {
    if (!confirm(`Send "${c.subject}" to all subscribers?`)) return;
    startTransition(async () => {
      const res = await sendNewsletter(c.id);
      if (res.ok) { toast.success(`Sent to ${res.recipients} subscribers`); router.refresh(); }
      else toast.error(res.message ?? "Error");
    });
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Subscribers" value={subscribers.length} icon={<Users className="h-6 w-6" />} accent="#6366f1" />
      ...
      {tab === "campaigns" ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {campaigns.map((c) => (
                <Panel key={c.id} className="flex flex-col">
                  ...
                  <p className="mt-2 text-xs text-muted">{c.status === "published" ? `Sent ${formatDate(c.sent_at)} to ${c.recipients ?? 0}` : `Created ${formatDate(c.created_at)}`}</p>
                  <div className="mt-3 flex gap-1.5">
                    {c.status !== "published" && (
                      <>
                        <button onClick={() => send(c)} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-brand-600 px-3 text-sm font-semibold text-white hover:bg-brand-700"><Send className="h-3.5 w-3.5" /> Send</button>
        ...
        <>
          {subscribers.length === 0 ? (
            ...
            <TableWrap>
              <thead className="border-b border-brand-100 bg-brand-50/40"><tr><Th>Email</Th><Th>Name</Th><Th>Status</Th><Th>Joined</Th><Th className="text-right">Actions</Th></tr></thead>
